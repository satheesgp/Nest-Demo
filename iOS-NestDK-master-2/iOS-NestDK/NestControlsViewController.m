...
@property (nonatomic, strong) ControllersTVC * contrlTVC;
@end
...
//    UIStoryboard * storyboard = [UIStoryboard storyboardWithName:@"ControllerStoryBoard" bundle:nil];
//    
//    self.contrlTVC = (ControllersTVC*)[storyboard instantiateViewControllerWithIdentifier:@"ControllerID"];
//    [self.navigationController presentViewController:self.contrlTVC animated:YES completion:nil];
}
...
{
    self.thermostatView = [[ThermostatView alloc] initWithFrame:CGRectMake(10, 10, 300, 250)];
...
- (void)structureUpdated:(NSDictionary *)structure
{
    ...
    if ([self.currentStructure objectForKey:@"thermostats"]) {
        ...
        [self.contrlTVC setDataArray:[self.currentStructure objectForKey:@"thermostats"]];
        [self.contrlTVC.tableView reloadData];
