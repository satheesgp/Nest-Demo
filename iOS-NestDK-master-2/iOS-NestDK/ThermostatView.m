...
@property (nonatomic, strong) UILabel *humiditySuffix;
...
#define HUMIDITY_SUFFIX @"humidity"
...
- (void)setHumidityValue:(NSInteger)humidityValue
{
    ...
    [self updateHumidityLabel:humidityValue];
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.humidityLabel = [self setupHumidityLabel:HUMIDITY_Y_LEVEL];
...
- (UILabel *)setupHumidityLabel:(int)yValue
{
    // Find out how wide to make the string
    NSDictionary *attributes = @{NSFontAttributeName: [UIFont fontWithName:BOLD_FONT size:TEMP_FONT_SIZE]};
    CGSize textSize = [PLACEHOLDER_TEXT sizeWithAttributes:attributes];
...
- (void)updateHumidityLabel:(NSInteger)newHumidityValue
{
    NSString *newString = [NSString stringWithFormat:@"%d%%", (int)newHumidityValue];
    
    NSDictionary *attributes = @{NSFontAttributeName: [UIFont fontWithName:BOLD_FONT size:TEMP_FONT_SIZE]};
    CGSize textSize = [newString sizeWithAttributes:attributes];
    
    [self.humidityLabel setFrame:CGRectMake(DEFAULT_PADDING, HUMIDITY_Y_LEVEL, textSize.width, TEMP_HEIGHT)];
    [self.humidityLabel setText:newString];
    [self.humiditySuffix setFrame:CGRectMake(DEFAULT_PADDING + 5 + textSize.width, HUMIDITY_Y_LEVEL + SUFFIX_HEIGHT - 3, SUFFIX_WIDTH, SUFFIX_HEIGHT)];
}
...
- (void)updateWithThermostat:(Thermostat *)thermostat
{
    ...
    self.humidityValue = thermostat.humidityValue;
